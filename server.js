...
// POST: guardar nuevo vehículo (requiere autenticación)
app.post('/api/cars', authenticateToken, async (req, res) => {
  const newCar = {
    ...req.body,
    ownerId: req.user.userId, // ← se obtiene del token JWT
  };
...
// DELETE: eliminar vehículo por ID (requiere admin)
app.delete('/api/cars/:id', authenticateToken, checkAdminRole, async (req, res) => {
  try {
